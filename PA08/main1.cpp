...
void KMPSearch(string pat, string txt)
{
    ...
    while (i < N) {
        KMP_counter++; // Increment comparison counter for the comparison pat[j] == txt[i]
        if (pat[j] == txt[i]) {
            j++;
            i++;
        }
...
void Boyer_Moore(string pat, string txt) {
    ...
    while (s <= (n - m)) {
        ...
        while (j >= 0 && pat[j] == txt[s + j]) {
            Boyer_Moore_counter++; // Increment comparison counter
            j--;
        }
...
int main() {
    ...
    cout << "Rabin Karp : " << rabin_karp_counter << endl;
    cout << "KMP\t : " << KMP_counter << endl;
    cout << "Boyer Moore : " << Boyer_Moore_counter << endl;
